...
export function ProductsClient({ products, categories }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    return list;
  }, [products, selectedCategories, selectedBrands, minRating, priceRange, sort]);

  const hasFilters   = selectedCategories.size > 0 || selectedBrands.size > 0 || minRating > 0 || priceRange[0] > 0 || priceRange[1] < 99999;
  const filterCount  = selectedCategories.size + selectedBrands.size + (minRating > 0 ? 1 : 0) + (priceRange[0] > 0 || priceRange[1] < 99999 ? 1 : 0);
    ...
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20, background: "white", borderRadius: 8, border: "1px solid #e2e8f0", padding: "10px 16px", flexWrap: "wrap" }}>
        ...
        <span style={{ fontSize: 13, color: "#64748b" }}>
          <strong style={{ color: "#0f172a" }}>{filtered.length}</strong> of {products.length} products
          {hasFilters && (
      ...
      <div style={{
        ...
        <div style={{ flex: 1, padding: "0 20px", overflowY: "auto" }}>
          ...
          <FilterSection title="Category">
            {categories.map(cat => (
              <label key={cat} style={{ display: "flex", alignItems: "center", gap: 10, padding: "6px 0", cursor: "pointer", fontSize: 14, color: selectedCategories.has(cat) ? "var(--color-accent)" : "#374151", fontWeight: selectedCategories.has(cat) ? 700 : 400 }}>
                <input type="checkbox" checked={selectedCategories.has(cat)} onChange={() => toggleSet(selectedCategories, setSelectedCategories, cat)} style={{ accentColor: "var(--color-accent)", width: 15, height: 15 }} />
                {cat}
              </label>
            ))}
          </FilterSection>
            ...
            <FilterSection title="Brand">
              {brands.map(brand => (
                <label key={brand} style={{ display: "flex", alignItems: "center", gap: 10, padding: "6px 0", cursor: "pointer", fontSize: 14, color: selectedBrands.has(brand) ? "var(--color-accent)" : "#374151", fontWeight: selectedBrands.has(brand) ? 700 : 400 }}>
                  <input type="checkbox" checked={selectedBrands.has(brand)} onChange={() => toggleSet(selectedBrands, setSelectedBrands, brand)} style={{ accentColor: "var(--color-accent)", width: 15, height: 15 }} />
                  {brand}
                </label>
              ))}
            </FilterSection>
        ...
        <div style={{ padding: "16px 20px", borderTop: "1px solid #e2e8f0", flexShrink: 0 }}>
          <button onClick={() => setDrawerOpen(false)} style={{ width: "100%", background: "var(--color-accent)", color: "white", border: "none", borderRadius: 8, padding: "12px", fontSize: 14, fontWeight: 700, cursor: "pointer" }}>
