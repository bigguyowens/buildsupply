...
function SubRow({ sub, onSaved, onDeleted }: { sub: AdminSubcategory; onSaved: () => void; onDeleted: () => void }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 0", borderBottom: "1px solid var(--ad-border2)" }}>
      {editing ? (
        <>
          ...
          <button onClick={() => { setEditing(false); setName(sub.name); }} style={{ padding: "5px 10px", borderRadius: 5, border: "1px solid var(--ad-border)", background: "var(--ad-surface)", fontSize: 12, cursor: "pointer", color: "var(--ad-muted)" }}>Cancel</button>
          {err && <span style={{ color: "#ef4444", fontSize: 12 }}>{err}</span>}
        </>
      ) : (
        <>
          <span style={{ flex: 1, fontSize: 13, color: "var(--ad-text2)" }}>{sub.name}</span>
          <span style={{ fontSize: 11, color: "var(--ad-muted2)" }}>{sub.slug}</span>
          <button onClick={() => setEditing(true)} style={{ padding: "4px 10px", borderRadius: 5, border: "1px solid var(--ad-border)", background: "var(--ad-surface)", fontSize: 11, fontWeight: 600, cursor: "pointer", color: "var(--ad-text2)" }}>Edit</button>
...
function AddSubForm({ categoryId, onAdded }: { categoryId: number; onAdded: () => void }) {
  ...
  if (!open) return (
    <button onClick={() => setOpen(true)} style={{ marginTop: 8, padding: "6px 14px", borderRadius: 6, border: "1px dashed #d1d5db", background: "var(--ad-surface)", fontSize: 12, fontWeight: 600, cursor: "pointer", color: "var(--ad-muted)" }}>
      + Add Subcategory
    </button>
  );
  ...
  return (
    <div style={{ marginTop: 10, display: "flex", gap: 8, alignItems: "center" }}>
      ...
      <button onClick={() => { setOpen(false); setName(""); }} style={{ padding: "7px 12px", borderRadius: 6, border: "1px solid var(--ad-border)", background: "var(--ad-surface)", fontSize: 13, cursor: "pointer", color: "var(--ad-muted)" }}>Cancel</button>
      {err && <span style={{ color: "#ef4444", fontSize: 12 }}>{err}</span>}
    </div>
  );
}
...
function CategoryRow({ cat, onRefresh }: { cat: AdminCategory; onRefresh: () => void }) {
  ...
  return (
    <div style={{ background: "var(--ad-surface)", borderRadius: 8, border: `1px solid ${open ? "#e2e8f0" : "#e2e8f0"}`, overflow: "hidden", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>

      {/* Row header */}
      <div style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 20px" }}>
        <button onClick={expand} style={{ background: "none", border: "none", cursor: "pointer", padding: 0, color: "var(--ad-muted2)", display: "flex", flexShrink: 0 }}>
        ...
        <button onClick={expand} style={{ flex: 1, background: "none", border: "none", cursor: "pointer", textAlign: "left", padding: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
            <span style={{ fontWeight: 700, fontSize: 15, color: "var(--ad-text)" }}>{cat.name}</span>
            <span style={{ fontSize: 11, color: "var(--ad-muted2)", background: "var(--ad-surface2)", padding: "2px 8px", borderRadius: 4, border: "1px solid var(--ad-border)" }}>{cat.slug}</span>
            <span style={{ fontSize: 12, color: "var(--ad-muted)" }}>{cat.productCount} products</span>
            <span style={{ fontSize: 12, color: "var(--ad-muted)" }}>{cat.subCount} subcategories</span>
            {cat.description && <span style={{ fontSize: 12, color: "var(--ad-muted2)", fontStyle: "italic" }}>{cat.description.slice(0, 60)}{cat.description.length > 60 ? "…" : ""}</span>}
          </div>
        </button>
        ...
        <button onClick={() => { setEditing(e => !e); setOpen(true); if (!open) { setLoadingSubs(true); getSubcategoriesForCategory(cat.id).then(d => { setSubs(d); setLoadingSubs(false); }); } }}
          style={{ padding: "6px 14px", borderRadius: 6, border: "1px solid var(--ad-border)", background: editing ? "#fff7ed" : "white", color: editing ? "var(--color-accent)" : "#374151", fontSize: 12, fontWeight: 700, cursor: "pointer", flexShrink: 0 }}>
      ...
      {/* Expanded content */}
      {open && (
        <div style={{ borderTop: "1px solid var(--ad-border2)", padding: "20px 24px", display: "flex", flexDirection: "column", gap: 20 }}>

          {/* Edit form */}
          {editing && (
            <div style={{ background: "var(--ad-surface2)", borderRadius: 8, border: "1px solid var(--ad-border)", padding: "18px 20px" }}>
              <p style={{ fontSize: 13, fontWeight: 700, color: "var(--ad-text)", margin: "0 0 14px" }}>Edit Category</p>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                <button onClick={() => setEditing(false)} style={{ padding: "8px 16px", borderRadius: 6, border: "1px solid var(--ad-border)", background: "var(--ad-surface)", fontSize: 13, cursor: "pointer", color: "var(--ad-muted)" }}>Cancel</button>
              </div>
            </div>
          )}
          ...
          {/* Subcategories */}
          <div>
            <p style={{ fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", color: "var(--ad-muted2)", margin: "0 0 10px" }}>Subcategories</p>
            {loadingSubs ? (
              <p style={{ fontSize: 13, color: "var(--ad-muted2)" }}>Loading…</p>
            ) : subs.length === 0 ? (
              <p style={{ fontSize: 13, color: "var(--ad-muted2)", fontStyle: "italic" }}>No subcategories yet.</p>
            ) : (
              <div>
                {subs.map(sub => (
    ...
    <>
      ...
      <div style={{ position: "fixed", top: 0, right: 0, bottom: 0, width: 440, maxWidth: "95vw", background: "var(--ad-surface)", zIndex: 201, boxShadow: "-4px 0 32px rgba(0,0,0,0.15)", display: "flex", flexDirection: "column" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "18px 24px", borderBottom: "1px solid var(--ad-border)", flexShrink: 0 }}>
          <span style={{ fontSize: 16, fontWeight: 800, color: "var(--ad-text)" }}>New Category</span>
          <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--ad-muted)" }}>
        ...
        <div style={{ flex: 1, padding: "20px 24px", overflowY: "auto", display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 11, color: "var(--ad-muted2)", margin: "4px 0 0" }}>Auto-generated from name. Used in URLs.</p>
          </div>
          <div>
            <label style={lbl}>Description</label>
    ...
    <div style={{ padding: "28px 32px", maxWidth: 1100 }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 24 }}>
        <div>
          ...
          <h1 style={{ fontSize: 22, fontWeight: 800, margin: "0 0 4px", color: "var(--ad-text)" }}>Categories</h1>
          <p style={{ color: "var(--ad-muted)", fontSize: 13, margin: 0 }}>{categories.length} categories · click a row to expand subcategories</p>
        </div>
