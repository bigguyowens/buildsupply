...
const inp: React.CSSProperties = { width: "100%", padding: "8px 11px", borderRadius: 6, border: "1px solid #e2e8f0", fontSize: 13, boxSizing: "border-box", outline: "none", fontFamily: "inherit" };
const lbl: React.CSSProperties = { fontSize: 11, fontWeight: 700, textTransform: "uppercase" as const, letterSpacing: "0.07em", color: "#64748b", display: "block", marginBottom: 4 };
    ...
    <div style={{ padding: "20px 24px", display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <div>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          ...
          <button onClick={() => { set("slug", toSlug(form.name)); setSlugEdited(true); }} style={{ padding: "7px 10px", borderRadius: 5, border: "1px solid #e2e8f0", background: "white", fontSize: 11, fontWeight: 700, cursor: "pointer", color: "#64748b", whiteSpace: "nowrap" }}>Reset</button>
        </div>
      </div>
      <div>
      ...
      <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 4 }}>
        <button onClick={onDone} style={{ padding: "7px 14px", borderRadius: 6, border: "1px solid #e2e8f0", background: "white", fontSize: 12, fontWeight: 700, cursor: "pointer", color: "#64748b" }}>Cancel</button>
    ...
    <div style={{ padding: "28px 32px", maxWidth: 800 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <Link href="/admin/blog" style={{ color: "#94a3b8", textDecoration: "none", fontSize: 13 }}>← Blog Posts</Link>
          <h1 style={{ fontSize: 20, fontWeight: 800, margin: 0, color: "#0f172a" }}>Blog Categories</h1>
        </div>
      ...
      {/* Add new form */}
      {adding && (
        <div style={{ background: "white", borderRadius: 10, border: "2px solid var(--color-accent)", marginBottom: 16 }}>
          <div style={{ padding: "12px 24px", borderBottom: "1px solid #e2e8f0" }}>
            <p style={{ margin: 0, fontWeight: 700, fontSize: 13, color: "#0f172a" }}>New Category</p>
          </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {cats.map(cat => (
          <div key={cat.id} style={{ background: "white", borderRadius: 10, border: "1px solid #e2e8f0", overflow: "hidden" }}>
            {editing === cat.id ? (
              <>
                <div style={{ padding: "12px 24px", borderBottom: "1px solid #e2e8f0", background: "#f8fafc" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 20px" }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <code style={{ fontSize: 11, background: "#f1f5f9", padding: "1px 6px", borderRadius: 4, color: "#64748b" }}>{cat.slug}</code>
                  </div>
                  {cat.description && <p style={{ margin: "2px 0 0", fontSize: 12, color: "#94a3b8" }}>{cat.description}</p>}
                  {delErr[cat.id] && <p style={{ margin: "4px 0 0", fontSize: 12, color: "#ef4444" }}>{delErr[cat.id]}</p>}
                </div>
                <div style={{ display: "flex", gap: 6 }}>
                  <button onClick={() => { setEditing(cat.id); setAdding(false); }} style={{ padding: "5px 12px", borderRadius: 5, border: "1px solid #e2e8f0", background: "white", fontSize: 11, fontWeight: 700, cursor: "pointer", color: "#374151" }}>Edit</button>
