...
function UsesDrawer({ promo, onClose }: { promo: Promotion; onClose: () => void }) {
  ...
  return (
    <>
      ...
      <div style={{ position: "fixed", top: 0, right: 0, bottom: 0, width: 480, maxWidth: "95vw", background: "white", zIndex: 201, boxShadow: "-4px 0 32px rgba(0,0,0,0.15)", display: "flex", flexDirection: "column" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "18px 24px", borderBottom: "1px solid #e2e8f0", flexShrink: 0 }}>
          <div>
            <span style={{ fontSize: 16, fontWeight: 800, color: "#0f172a" }}>{promo.code}</span>
            <span style={{ marginLeft: 10, fontSize: 13, color: "#64748b" }}>{promo.discount_percent}% off · {promo.used_count} use{promo.used_count !== 1 ? "s" : ""}</span>
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 24px" }}>
          ...
          {uses && uses.length === 0 && <p style={{ color: "#94a3b8", fontSize: 14, fontStyle: "italic" }}>No uses yet.</p>}
          {uses && uses.length > 0 && (
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              <thead>
                <tr style={{ background: "#f8fafc" }}>
                  {["Customer", "Order", "Date"].map(h => (
                    <th key={h} style={{ padding: "8px 12px", textAlign: "left", fontSize: 11, fontWeight: 700, textTransform: "uppercase", color: "#94a3b8", borderBottom: "1px solid #e2e8f0" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                    ...
                    <td style={{ padding: "10px 12px" }}>
                      {u.user_name ? (
                        <div>
                          ...
                          <p style={{ margin: 0, fontSize: 11, color: "#94a3b8" }}>{u.user_email}</p>
                        </div>
  ...
  return (
    <>
      ...
      <div style={{ position: "fixed", top: 0, right: 0, bottom: 0, width: 440, maxWidth: "95vw", background: "white", zIndex: 201, boxShadow: "-4px 0 32px rgba(0,0,0,0.15)", display: "flex", flexDirection: "column" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "18px 24px", borderBottom: "1px solid #e2e8f0", flexShrink: 0 }}>
          <span style={{ fontSize: 16, fontWeight: 800, color: "#0f172a" }}>New Promo Code</span>
          <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", color: "#64748b", fontSize: 20 }}>✕</button>
        </div>
        <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px", display: "flex", flexDirection: "column", gap: 14 }}>
          <div>
            ...
            <p style={{ fontSize: 11, color: "#94a3b8", margin: "4px 0 0" }}>Auto-uppercased. Customers enter this at checkout.</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p style={{ fontSize: 11, color: "#94a3b8", margin: "4px 0 0" }}>Leave blank for no expiry.</p>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px", background: "#f8fafc", borderRadius: 8, border: "1px solid #e2e8f0" }}>
            ...
            <div>
              <label htmlFor="opc" style={{ fontSize: 13, fontWeight: 700, color: "#0f172a", cursor: "pointer" }}>One per customer</label>
              <p style={{ fontSize: 11, color: "#94a3b8", margin: "2px 0 0" }}>Logged-in customers can only use this code once.</p>
            </div>
          </div>
    ...
    <div style={{ padding: "28px 32px", maxWidth: 1100 }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 24 }}>
        <div>
          ...
          <h1 style={{ fontSize: 22, fontWeight: 800, margin: "0 0 4px", color: "#0f172a" }}>Promotions</h1>
          <p style={{ color: "#64748b", fontSize: 13, margin: 0 }}>Percentage-off promo codes with one-per-customer enforcement</p>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginBottom: 24 }}>
        {[
          ...
          <div key={s.label} style={{ background: "white", borderRadius: 8, padding: "16px 20px", border: "1px solid #e2e8f0", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
      ...
      <div style={{ background: "white", borderRadius: 10, border: "1px solid #e2e8f0", overflow: "hidden", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
        {promos.length === 0 ? (
          <p style={{ padding: "48px", textAlign: "center", color: "#94a3b8", fontSize: 14 }}>No promo codes yet. Create your first one!</p>
        ) : (
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr style={{ background: "#f8fafc" }}>
                ...
                return (
                  <tr key={promo.id} style={{ borderTop: "1px solid #f1f5f9" }}>
                    <td style={{ padding: "14px 16px" }}>
                      <span style={{ fontWeight: 800, fontSize: 14, letterSpacing: "0.06em", color: "#0f172a", fontFamily: "monospace", background: "#f8fafc", padding: "2px 8px", borderRadius: 4, border: "1px solid #e2e8f0" }}>
                        {promo.code}
                      </span>
                      {promo.one_per_customer && <span style={{ marginLeft: 6, fontSize: 10, color: "#94a3b8" }}>1x/customer</span>}
                    </td>
                    <td style={{ padding: "14px 16px", color: "#64748b", maxWidth: 200 }}>{promo.description || "—"}</td>
                    ...
                    <td style={{ padding: "14px 16px", color: "#64748b" }}>{promo.max_uses ?? "∞"}</td>
                    ...
                    <td style={{ padding: "14px 16px" }}>
                      <div style={{ display: "flex", gap: 6 }}>
                        <button onClick={() => handleToggle(promo.id, !promo.active)}
                          style={{ padding: "5px 10px", borderRadius: 5, border: "1px solid #e2e8f0", background: "white", fontSize: 11, fontWeight: 700, cursor: "pointer", color: promo.active ? "#64748b" : "#15803d" }}>
