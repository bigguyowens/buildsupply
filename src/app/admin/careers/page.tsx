    ...
    <div style={{ padding: 28 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: "var(--ad-muted2)", margin: "4px 0 0" }}>Manage job postings and applications</p>
        </div>
        <div style={{ display: "flex", gap: 10 }}>
          <Link href="/careers" target="_blank" style={{ padding: "9px 16px", borderRadius: 8, background: "var(--ad-surface)", border: "1px solid var(--ad-border)", color: "var(--ad-text2)", fontWeight: 600, fontSize: 13, textDecoration: "none" }}>
            View Public Page ↗
          </Link>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 24 }}>
        {[
          { label: "Total Postings",  value: postings.length,                          color: "var(--ad-text)" },
      ...
      <div style={{ background: "var(--ad-surface)", borderRadius: 10, boxShadow: "0 1px 4px rgba(0,0,0,0.06)", overflow: "hidden" }}>
        {postings.length === 0 ? (
          <div style={{ padding: "60px 20px", textAlign: "center" }}>
            <p style={{ color: "var(--ad-muted2)", fontSize: 15, margin: "0 0 16px" }}>No postings yet</p>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            ...
            <tbody>
              {postings.map(p => {
                ...
                return (
                  <tr key={p.id} style={{ borderTop: "1px solid var(--ad-border2)" }}>
                    <td style={{ padding: "12px 16px", fontWeight: 700 }}>{p.title}</td>
                    <td style={{ padding: "12px 16px", color: "var(--ad-muted)" }}>{p.department}</td>
                    <td style={{ padding: "12px 16px", color: "var(--ad-muted)" }}>{p.type}</td>
                    <td style={{ padding: "12px 16px", color: "var(--ad-muted)" }}>{p.location}</td>
                    ...
                    <td style={{ padding: "12px 16px", color: "var(--ad-muted2)", fontSize: 12 }}>
                      {new Date(p.created_at).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}
                    </td>
                    <td style={{ padding: "12px 16px" }}>
                      <div style={{ display: "flex", gap: 10 }}>
                        <Link href={`/admin/careers/${p.id}`} style={{ color: "#f97316", fontWeight: 700, fontSize: 12, textDecoration: "none" }}>View</Link>
                        <Link href={`/admin/careers/${p.id}/edit`} style={{ color: "var(--ad-muted)", fontWeight: 700, fontSize: 12, textDecoration: "none" }}>Edit</Link>
                      </div>
                    </td>
                  </tr>
                );
