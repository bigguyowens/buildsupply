...
const lbl: React.CSSProperties = { fontSize: 11, fontWeight: 700, textTransform: "uppercase" as const, letterSpacing: "0.08em", color: "var(--ad-muted)", display: "block", marginBottom: 6 };
const card: React.CSSProperties = { background: "var(--ad-surface)", borderRadius: 10, border: "1px solid var(--ad-border)", padding: "20px 24px" };
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        <div style={{ position: "relative", width: 40, height: 40, borderRadius: 8, overflow: "hidden", border: "1px solid var(--ad-border)", flexShrink: 0 }}>
        ...
        <input type="text" value={value} onChange={e => onChange(e.target.value)} maxLength={7}
          style={{ flex: 1, padding: "8px 11px", borderRadius: 6, border: "1px solid var(--ad-border)", fontSize: 13, fontFamily: "monospace", outline: "none" }} />
      </div>
    </div>
  );
}
...
function Preview({ theme }: { theme: SiteTheme }) {
  ...
  return (
    <div style={{ border: "1px solid var(--ad-border)", borderRadius: 10, overflow: "hidden", fontSize: 12, boxShadow: "0 4px 20px rgba(0,0,0,0.08)" }}>
      {/* Nav bar */}
      ...
      <div style={{ background: theme.color_background, padding: "14px 16px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
          {["Concrete Mix", "Safety Harness", "Power Drill"].map(name => (
            <div key={name} style={{ background: "var(--ad-surface)", borderRadius: 6, border: "1px solid var(--ad-border)", padding: "10px 8px" }}>
              <div style={{ height: 36, background: "var(--ad-surface2)", borderRadius: 4, marginBottom: 6 }} />
              <p style={{ fontFamily: bFont, fontSize: 10, fontWeight: 700, color: theme.color_foreground, margin: "0 0 2px" }}>{name}</p>
              <p style={{ fontFamily: bFont, fontSize: 9, color: "var(--ad-muted)", margin: "0 0 6px" }}>$24.99</p>
  ...
  return (
    <div style={{ padding: "28px 32px", maxWidth: 1100 }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 28 }}>
        <div>
          <p style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: "var(--ad-muted2)", margin: "0 0 4px" }}>Admin · Settings</p>
          <h1 style={{ fontSize: 22, fontWeight: 800, margin: "0 0 4px", color: "var(--ad-text)" }}>Site Theme</h1>
          <p style={{ color: "var(--ad-muted)", fontSize: 13, margin: 0 }}>Customize colors and typography across the entire site</p>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 360px", gap: 24, alignItems: "start" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div style={card}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {COLOR_PRESETS.map(preset => (
                <button key={preset.label} onClick={() => applyPreset(preset)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 12px", borderRadius: 7, border: "1px solid var(--ad-border)", background: "var(--ad-surface)", cursor: "pointer", fontSize: 12, fontWeight: 600, color: "var(--ad-text2)" }}>
          ...
          <div style={card}>
            ...
            <div style={{ marginTop: 16, padding: "10px 14px", background: "var(--ad-surface2)", borderRadius: 6, border: "1px solid var(--ad-border)" }}>
              <p style={{ fontSize: 11, color: "var(--ad-muted2)", margin: 0 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
              <div>
                ...
                <select value={theme.heading_font} onChange={e => set("heading_font", e.target.value)}
                  style={{ width: "100%", padding: "9px 11px", borderRadius: 6, border: "1px solid var(--ad-border)", fontSize: 13, cursor: "pointer", outline: "none", background: "var(--ad-surface)" }}>
                  {HEADING_FONTS.map(f => <option key={f.value} value={f.value}>{f.label}</option>)}
                </select>
                <p style={{ margin: "8px 0 0", fontFamily: `'${theme.heading_font}', system-ui, sans-serif`, fontSize: 20, fontWeight: 800, color: "var(--ad-text)", lineHeight: 1 }}>
                  The quick brown fox
                </p>
              </div>
              <div>
                ...
                <p style={{ margin: "8px 0 0", fontFamily: `'${theme.body_font}', system-ui, sans-serif`, fontSize: 14, color: "var(--ad-text2)", lineHeight: 1.6 }}>
                  Industrial-grade construction supplies for contractors and project managers.
                </p>
              </div>
            </div>
            <div style={{ marginTop: 14, padding: "10px 14px", background: "var(--ad-surface2)", borderRadius: 6, border: "1px solid var(--ad-border)" }}>
              <p style={{ fontSize: 11, color: "var(--ad-muted2)", margin: 0 }}>
          ...
          <div style={{ textAlign: "right" }}>
            <button onClick={() => setTheme({ color_primary: "#002244", color_accent: "#e8561c", color_background: "#f4f5f6", color_foreground: "#111827", heading_font: "Geist", body_font: "Geist" })}
              style={{ padding: "6px 14px", borderRadius: 6, border: "1px solid var(--ad-border)", background: "var(--ad-surface)", fontSize: 11, fontWeight: 700, cursor: "pointer", color: "var(--ad-muted)" }}>
              Reset to Defaults
            </button>
          </div>
        </div>
        ...
        <div style={{ position: "sticky", top: 24 }}>
          ...
          <Preview theme={theme} />
          <p style={{ fontSize: 11, color: "var(--ad-muted2)", marginTop: 10, textAlign: "center" }}>
