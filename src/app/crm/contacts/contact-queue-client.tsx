  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 420px", gap: 20, alignItems: "start" }}>
      ...
      <div style={{ background: "var(--crm-surface)", borderRadius: 10, border: "1px solid var(--crm-border)", overflow: "hidden" }}>
        {/* Tabs */}
        <div style={{ display: "flex", borderBottom: "1px solid var(--crm-border2)", padding: "0 16px" }}>
          {tabs.map(t => (
        ...
        {filtered.length === 0 ? (
          <p style={{ color: "var(--crm-muted2)", fontSize: 14, padding: "40px 24px", textAlign: "center" }}>
          ...
          <div key={c.id} onClick={() => { setSelected(c); setReplyBody(""); }}
            ...
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
              <span style={{ fontWeight: 700, fontSize: 13, color: "var(--crm-text)" }}>{c.name}</span>
            ...
            <p style={{ fontSize: 12, fontWeight: 600, color: "var(--crm-text2)", margin: "0 0 2px" }}>{c.reason ?? "General Inquiry"}</p>
      ...
      <div style={{ background: "var(--crm-surface)", borderRadius: 10, border: "1px solid var(--crm-border)", overflow: "hidden" }}>
        {!selected ? (
          <div style={{ padding: "48px 24px", textAlign: "center", color: "var(--crm-muted2)" }}>
            <p style={{ fontSize: 32, margin: "0 0 8px" }}>✉️</p>
            <p style={{ fontSize: 14, fontWeight: 600, margin: 0 }}>Select a submission to reply</p>
          </div>
        ) : (
          <div>
            <div style={{ padding: "14px 18px", borderBottom: "1px solid var(--crm-border2)", background: "#0d0d0d" }}>
              <p style={{ fontWeight: 800, fontSize: 14, color: "#f5c700", margin: "0 0 2px" }}>{selected.reason ?? "General Inquiry"}</p>
              <p style={{ fontSize: 12, color: "var(--crm-muted)", margin: 0 }}>{selected.name} · {selected.email}</p>
            </div>
            <div style={{ padding: "16px 18px", borderBottom: "1px solid var(--crm-border2)",
              background: "var(--crm-surface2)", maxHeight: 180, overflowY: "auto" }}>
              <p style={{ fontSize: 13, color: "var(--crm-text2)", margin: 0, lineHeight: 1.7,
                whiteSpace: "pre-wrap" }}>{selected.message}</p>
            </div>
            <div style={{ padding: 18 }}>
              <p style={{ fontSize: 11, fontWeight: 800, textTransform: "uppercase",
                letterSpacing: "0.08em", color: "var(--crm-muted2)", margin: "0 0 8px" }}>Your Reply</p>
