...
export function CRMOrdersClient({ orders, totalRevenue, statusCounts, sessionRole, scope, scopeToggle }: {
  orders: CRMOrder[];
  totalRevenue: number;
      ...
      <div style={{ background: "var(--crm-surface)", borderRadius: 10, border: "1px solid var(--crm-border)", overflow: "hidden" }}>
        ...
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={9} style={{ padding: "48px 24px", textAlign: "center", color: "var(--crm-muted2)" }}>
                  No orders found
                </td></tr>
              ) : filtered.map((o, i) => {
                const meta = STATUS_META[o.status] ?? { label: o.status, color: "var(--crm-muted)", bg: "#f1f1f1" };
                return (
                  <tr key={o.id} style={{ borderBottom: "1px solid var(--crm-border2)",
                    ...
                    <td style={{ padding: "12px 16px" }}>
                      ...
                      <span style={{ fontSize: 11, color: "var(--crm-muted2)" }}>{o.email}</span>
                    </td>
                    ...
                    {/* Date */}
                    <td style={{ padding: "12px 16px", color: "var(--crm-muted)", whiteSpace: "nowrap" as const }}>
